...
const nodeId = `worker-${os_1.default.hostname()}-${Math.random().toString(36).substring(2, 7)}`;
let token = '';
let workerId = '';
let projectId = '';
let isPolling = false;
...
let socket = null;
const runningJobAborts = new Map(); // Cancel callback map
...
async function start() {
    ...
    try {
        if (WORKER_TOKEN) {
            ...
            const loginRes = await axios_1.default.post(`${MASTER_URL}/api/v1/auth/login`, {
                email: ADMIN_EMAIL,
            ...
            const projectsRes = await axios_1.default.get(`${MASTER_URL}/api/v1/projects`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            const defaultProject = projectsRes.data.find((p) => p.slug === 'default-project') || projectsRes.data[0];
            if (!defaultProject) {
        ...
        if (PROVIDER_ACCOUNT_ID)
            registerPayload.providerAccountId = PROVIDER_ACCOUNT_ID;
        if (WORKER_POOL_ID)
            registerPayload.workerPoolId = WORKER_POOL_ID;
        const registerRes = await axios_1.default.post(`${MASTER_URL}/api/v1/workers/register`, registerPayload, {
            headers: { Authorization: `Bearer ${token}` }
        });
...
function startHeartbeatLoop() {
    setInterval(async () => {
        try {
            ...
            await axios_1.default.post(`${MASTER_URL}/api/v1/workers/heartbeat`, {
                workerId,
...
function startPollingLoop() {
    setInterval(async () => {
        ...
        isPolling = true;
        try {
            const response = await axios_1.default.get(`${MASTER_URL}/api/v1/workers/${workerId}/tasks/next`);
...
async function executeJob(job) {
    ...
    const startLeaseRenewal = () => {
        leaseTimer = setInterval(async () => {
            try {
                await axios_1.default.post(`${MASTER_URL}/api/v1/jobs/${jobId}/lease/renew`, {
    ...
    const writeJobLog = async (level, message, metadata) => {
        try {
            await axios_1.default.post(`${MASTER_URL}/api/v1/jobs/${jobId}/logs`, {
                level,
    ...
    const writeJobProgress = async (progress, stage, message) => {
        try {
            await axios_1.default.post(`${MASTER_URL}/api/v1/jobs/${jobId}/progress`, {
                progress,
                stage,
                message,
            });
    ...
    try {
        ...
        if (jobType === 'sleep') {
            ...
            for (let i = 1; i <= progressSteps; i++) {
                if (isAborted)
                    throw new Error('Job execution cancelled by user request.');
            ...
            resultPayload = {
                ...
                hostNode: os_1.default.hostname(),
        ...
        await axios_1.default.post(`${MASTER_URL}/api/v1/jobs/${jobId}/result`, {
            leaseToken: lease.leaseToken,
            status: 'COMPLETED',
