...
export function registerRoutes(app: any) {
  ...
  const io: Server = app.locals.io;
  const jobQueue: Queue = app.locals.jobQueue;
  ...
  router.post('/auth/login', async (req: Request, res: Response) => {
    try {
      ...
      const token = jwt.sign(
        { userId: user.id, userRole: user.role, email: user.email },
        JWT_SECRET,
        { expiresIn: '24h' }
      );
  ...
  router.get('/workers', authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
  ...
  router.post('/jobs', authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
  ...
  router.get('/jobs/:jobId/logs', authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
  ...
  router.get('/workers/:workerId/tasks/next', async (req: Request, res: Response) => {
    try {
  ...
  router.post('/jobs/:jobId/lease/renew', async (req: Request, res: Response) => {
    try {
      const { jobId } = req.params;
  ...
  router.post('/jobs/:jobId/progress', async (req: Request, res: Response) => {
    try {
      const { jobId } = req.params;
  ...
  router.post('/jobs/:jobId/logs', async (req: Request, res: Response) => {
    try {
      const { jobId } = req.params;
  ...
  router.post('/jobs/:jobId/result', async (req: Request, res: Response) => {
    try {
      const { jobId } = req.params;
  ...
  router.post('/provider-accounts/link-cli', authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
